...
train_path = Path(__file__).parent/"Train.csv" #data/Train.csv
val_path = Path(__file__).parent/"Val.csv"

df_comments = load_and_prepare("comments")
df_web_data = load_and_prepare("web_data")
df_train = pd.read_csv(train_path)

df_merged = pd.concat([df_train,df_comments,df_web_data],ignore_index=True)
...
df_new_train.to_csv(train_path,index = False)
df_new_val.to_csv(val_path,index = False)
